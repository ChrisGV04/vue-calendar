...
import type { DayOfWeek, MonthGrid } from './types';
...
export function createMonth(props: CreateMonthProps): MonthGrid<DateValue> {
...
type SetMonthProps = CreateMonthProps & {
  ...
  currentMonths?: MonthGrid<DateValue>[];
};

export function createMonths(props: SetMonthProps) {
  const { numberOfMonths, dateObj, ...monthProps } = props;

  const months: MonthGrid<DateValue>[] = [];
